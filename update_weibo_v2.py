...
# 关键是：weibo.html 用 "weiboN.jpg" 文件名 + WEIBO_BASE 拼接
# 做法：把每个 "weiboN.jpg" 替换为完整新URL，同时改 map 函数
# 改 map 函数：如果 f 以 http 开头就直接用，否则 WEIBO_BASE + f

# 先把 .map(f => ({ img: WEIBO_BASE + f, text: f })) 改成支持完整URL
old_map = '.map(f => ({ img: WEIBO_BASE + f, text: f }))'
new_map = '.map(f => ({ img: f.indexOf("http")===0?f:WEIBO_BASE+f, text: f.replace(/^.*\\//,"") }))'
if old_map in c:
...
for num, new_url in NEW_URLS.items():
    ...
    if old_name in c:
        c = c.replace(old_name, f'"{new_url}"')
        count += 1
